    ...
    private void updateEstimate(boolean startJob) {
        try {
            if (cache_east != null &&
                ...
                long tilecount = mgr.possibleTilesInArea(bb, zoommin, zoommax);
                cache_estimate.setText(tilecount + " 张瓦片地图  预计共" +tilecount*6/1024+"MB");
                if (startJob)
                {
    ...
    private void showCurrentCacheInfo() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // set title
                alertDialogBuilder.setTitle(R.string.cache_manager)
                        .setMessage("Cache Capacity (MB): " + mgr.cacheCapacity()/1024/1024/8 + "MB \n"+
                                "Cache Usage (MB): " + mgr.currentCacheUsage()/1024/1024/8+"MB");
